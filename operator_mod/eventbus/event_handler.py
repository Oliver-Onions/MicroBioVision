...
class EventManager():
    ...
    def add_listener(self, event_name: str | EventKeys, listener: Callable, priority: int = 0, gui_safe: bool = False) -> None:
        ...
        print(f"🔔 Listener registered for event {event_name}: {listener.__name__} (GUI-safe: {gui_safe})")
    ...
    def trigger_event(self, event_name: str | EventKeys, *args, **kwargs) -> None:
        ...
        print(f"🚀 Triggering event: {event_name} with {len(listeners_to_notify)} listener(s).")
        for listener, _, _ in listeners_to_notify:
            print(f" - Listener: {listener.__name__}")
